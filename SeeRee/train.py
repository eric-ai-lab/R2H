...
import os
# os.environ['MASTER_PORT'] = '13345' # modify if RuntimeError: Address already in use
...
from SeeRee.configs.config import (basic_check_arguments, shared_configs, restore_training_settings)
from SeeRee import make_data_loader
from SeeRee.evalcap.utils_caption_evaluate import evaluate_on_coco_caption
from SeeRee.utils.logger import LOGGER as logger
from SeeRee.utils.logger import (TB_LOGGER, RunningMeter, add_log_to_file)
from SeeRee.utils.load_save import TrainingRestorer, TrainingSaver
from SeeRee.utils.comm import (is_main_process,
                            get_rank, get_world_size, dist_init)
from SeeRee.utils.miscellaneous import (NoOp, mkdir, set_seed, str_to_bool,
                                    delete_tsv_files, concat_tsv_files)
from SeeRee.utils.metric_logger import MetricLogger
from SeeRee.utils.tsv_file_ops import tsv_writer, reorder_tsv_keys
from SeeRee.utils.deepspeed import get_deepspeed_config, fp32_to_fp16
...
from SeeRee.solver import AdamW

from azureml.core.run import Run

# from HAMT.finetune_src.cvdn.main import build_dataset
# from HAMT.finetune_src.cvdn.main import valid
...
def mixed_precision_init(args, model):
    ...
    if args.mixed_precision_method == "deepspeed":
        deepspeed.init_distributed(distributed_port = os.getenv('MASTER_PORT'))
        ...
        if backward_now:
            ...
            if (args.save_steps > 0 and global_step % args.save_steps == 0) or global_step == max_global_step or global_step == 1:
                ...
                if args.evaluate_during_training:
                    ...
                    ############ here ###############
                    #################################
                    evaluate_file = evaluate(args, val_dataloader[1], model, tokenizer, checkpoint_dir)
                    if get_world_size() > 1:
                        dist.barrier()
                    if is_main_process():
                    ...
                    if get_world_size() > 1:
                        dist.barrier()       
                    evaluate_file = evaluate(args, val_dataloader[0], model, tokenizer, checkpoint_dir, val_seen=True)
...
def get_predict_file(output_dir, args, data_yaml_file, val_seen = False):
    cc = ['pred']
...
def evaluate(args, val_dataloader, model, tokenizer, output_dir, val_seen = False):
    predict_file = get_predict_file(output_dir, args,
            val_dataloader.dataset.yaml_file, val_seen = val_seen)
    ...
    if is_main_process():
        ...
        # #########################################################
        # ########## quick RDH eval during training ##############
        # #########################################################
        # if val_seen and 'CVDN' in args.data_dir:
        #     parser = ArgumentParser()
        #     args_hamt = parser.parse_args('')
        #     with open('./SwinBERT/eval_in_HAMT_cvdn.json', 'r') as f:
        #         args_hamt.__dict__ = json.load(f)
        #     args_hamt.test_swinbert_res = evaluate_file.replace('.eval','_coco_format')
        #     args_hamt.pred_dir = "./HAMT/datasets/" + args_hamt.test_swinbert_res.split('/')[-3] + '/' + evaluate_file.split('/')[-2].split('.')[0]
        #     args_hamt.log_dir = args_hamt.pred_dir + '/logs'
        #     os.makedirs(args_hamt.pred_dir, exist_ok=True)
        #     os.makedirs(args_hamt.log_dir, exist_ok=True)
        #     train_env, val_envs, aug_env = build_dataset(args_hamt, rank=0)
        #     valid(args_hamt, train_env, val_envs, rank=0)
...
def get_custom_args(base_config):
    ...
    parser.add_argument('--parsed', '-parsed', action='store_true', default=False)
    parser.add_argument('--no_cos_mask', '-no_cos_mask', action='store_true', default=False)
    parser.add_argument('--parsed_with_seperation', '-parsed_with_seperation', action='store_true', default=False)
...
def main(args):
    ...
    dist_init(args)  # Yue: set to 1 GPU
    ...
    if args.do_eval:
        ...
        args.pretrained_checkpoint = args.resume_checkpoint.split('model.bin')[0]
    ...
    if args.do_train and args.pretrained_checkpoint != '':
    ...
    ###################################
    ######## Make loader ##############
    train_dataloader = make_data_loader(args, args.data_dir, tokenizer, args.distributed, is_train=True)
    val_unseen_dataloader = make_data_loader(args, args.data_dir, tokenizer, args.distributed, is_train=False, val_seen=False)
    val_seen_dataloader = make_data_loader(args, args.data_dir, tokenizer, args.distributed, is_train=False, val_seen=True)
    ...
    if not args.do_eval:
        train(args, train_dataloader, (val_seen_dataloader, val_unseen_dataloader), vl_transformer, tokenizer, training_saver, optimizer, scheduler)

    else:
        # args, vl_transformer, _, _ = mixed_precision_init(args, vl_transformer)
        evaluate_file = evaluate(args, val_unseen_dataloader, vl_transformer, tokenizer, args.eval_model_dir)
        print(evaluate_file)
    if args.distributed:
