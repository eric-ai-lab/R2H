...
class CaptionTensorizer(object):
    def __init__(self, tokenizer, max_img_seq_length=50, max_seq_length=70, 
            ...
            attn_mask_type='seq2seq', is_train=True, mask_a_and_b=False,
        ...
        Args:
            ...
            mask_a_and_b: whether to mask text_b or not during training.
        """
        ...
        self.mask_a_and_b = mask_a_and_b
    ...
    def get_attn_masks(self, seq_a_len, seq_len, got_a_generate_b):
        # image features
        ...
        if self.is_train and self.attn_mask_type == 'bidirectional' \
            ...
            if got_a_generate_b:
                attention_mask[c_start : l_end, c_start : l_end].copy_(
                        self._triangle_mask[0 : l_end- c_start, 0 :  l_end- c_start]
                )
                # Yue: Question: c. generated answer: l. image seq: r.

                # Yue:  attention enabled for answer to r
                attention_mask[l_start : l_end, r_start : r_end] = 1 
                # Yue: full attention for C-C, R-R:
        ...
        else: # yue: just miss the triangle att than the learn_vid_att
            # prepare attention mask:
            # note that there is no attention from caption to image
            # because otherwise it will violate the triangle attention 
            # for caption as caption will have full attention on image. 
            ...
            if got_a_generate_b:
                # full attention for C-C, R-R
                attention_mask[c_start : c_end, c_start : c_end] = 1
                attention_mask[r_start : r_end, r_start : r_end] = 1
                # full attention for L-C, C-R
                attention_mask[l_start : l_end, c_start : c_end] = 1
                attention_mask[l_start : l_end, r_start : r_end] = 1
                # full attention for C-R:
                attention_mask[c_start : c_end, r_start : r_end] = 1
                attention_mask[r_start : r_end, c_start : c_end] = 1
            else:
    ...
    def get_text_mask_idx(self, seq_a_len, seq_len, text_meta=None, got_a_generate_b=False, qa_as_caption=False, ans_starting_pos = 0):
        ...
        if self.mask_a_and_b:
            # can mask both text_a and text_b
            num_masked = min(max(round(self.mask_prob * seq_len), 1), self.max_masked_tokens)
        elif got_a_generate_b:
            num_masked = min(max(round(self.mask_prob * (seq_len - self.max_seq_a_len)), 1), self.max_masked_tokens)
        elif qa_as_caption:
            num_masked = min(max(round(self.mask_prob * (seq_len - ans_starting_pos)), 1), self.max_masked_tokens)
        else:
            # only mask text_a
        ...
        if self.mask_a_and_b :
            ...
            text_b_candidate = list(range(self.max_seq_a_len, seq_len-1))
        elif got_a_generate_b:
            # text b always random masking
            text_b_candidate = list(range(self.max_seq_a_len+1, seq_len))
        else:
            text_b_candidate = []
        if self.text_mask_type == 'pos_tag' and random.random() > self.random_mask_prob \
            ...
            if got_a_generate_b:
                ...
                candidate_masked_idx = list(range(ans_starting_pos+1, seq_a_len))
            else:
    ...
    def mask_text_inputs(self, tokens, seq_a_len, seq_len, text_meta=None, got_a_generate_b=False, qa_as_caption=False):
        if self.is_train:
            if self.text_mask_type == "attn_on_the_fly" and random.random() > self.random_mask_prob and len(tokens)> 2 \
                ...
                masked_idx = self.get_text_mask_idx(seq_a_len, seq_len, text_meta, got_a_generate_b, qa_as_caption, self.ans_starting_pos)
                try:
            ...
            if got_a_generate_b:
                ...
                masked_pos[0:self.max_seq_a_len] -= 1
            elif qa_as_caption:
                masked_pos = torch.ones(self.max_seq_len, dtype=torch.int)
                masked_pos[0:self.ans_starting_pos] -= 1
            else:
                masked_pos = torch.ones(self.max_seq_len, dtype=torch.int)
            mlm_targets = None
    ...
    def tokenize_text_inputs(
            ...
            sequence_b_segment_id=1, text_meta=None, got_a_generate_b = False, qa_as_caption=False):
        ...
        elif not self.is_train and got_a_generate_b:
            tokens_a = self.tokenizer.tokenize(text_a)
        elif not self.is_train and qa_as_caption:
            tokens_a = self.tokenizer.tokenize(text_a)
        else:
        ...
        if qa_as_caption:
            ...
            if self.is_train:
                ...
                if len(tokens_b) > self.max_seq_len - len(tokens_a) - 2:
                    tokens_b = tokens_b[: (self.max_seq_len - len(tokens_a) - 2)]
            else:
                # fake tokens to generate masks
                tokens_b = [self.tokenizer.mask_token] * (self.max_seq_len - len(tokens_a) - 2)

            tokens = tokens_a + [self.tokenizer.cls_token] + tokens_b + [self.tokenizer.sep_token]
            
            ans_starting_pos = len(tokens_a)
            self.ans_starting_pos = ans_starting_pos
            segment_ids = [sequence_a_segment_id] * (len(tokens))
            segment_ids[ans_starting_pos] = cls_token_segment_id
            seq_a_len = len(tokens)
        elif got_a_generate_b:
            assert self.max_seq_len > self.max_seq_a_len
            text_b = self.prepro_raw_txt(text_b)
            ...
            if self.is_train:
                ...
                # fake tokens to generate masks
                tokens_b = [self.tokenizer.mask_token] * (self.max_seq_len - len(tokens) - 2)
            tokens += [self.tokenizer.cls_token] + tokens_b + [self.tokenizer.sep_token]  ## Yue: cls_token or others?
            segment_ids += [sequence_a_segment_id] * (len(tokens_b) + 2)
    ...
    def tensorize_example_e2e(self, text_a, img_feat, text_b=None,
            ...
            sequence_a_segment_id=0, sequence_b_segment_id=1, text_meta=None, got_a_generate_b = False, qa_as_caption=False):
        # tokenize the texts
        tokens, segment_ids, seq_a_len, seq_len = self.tokenize_text_inputs(
            text_a, text_b, cls_token_segment_id, pad_token_segment_id,
            sequence_a_segment_id, sequence_b_segment_id, text_meta, got_a_generate_b,qa_as_caption)
        
        # masking the tokens
        tokens_after_masking, masked_pos, mlm_targets = self.mask_text_inputs(
            tokens, seq_a_len, seq_len, text_meta, got_a_generate_b, qa_as_caption)
        ...
        attention_mask = self.get_attn_masks(seq_a_len, seq_len, got_a_generate_b)
...
def build_tensorizer(args, tokenizer, is_train=True):
    if hasattr(args, 'mask_od_labels'):
        mask_a_and_b = args.mask_od_labels
    else:
        mask_a_and_b = False
    if is_train:
    ...
    return CaptionTensorizer(
            tokenizer,
            max_img_seq_length=args.max_img_seq_length,
            max_seq_length=args.max_seq_length if args.add_od_labels or args.got_a_generate_b else args.max_gen_length,
            max_seq_a_length= args.max_seq_a_length if args.got_a_generate_b else args.max_gen_length,
            is_train=False,
            attn_mask_type=args.attn_mask_type,
    )
