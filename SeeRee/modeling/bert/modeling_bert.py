...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None,
        ...
        all_hidden_states = ()
        all_attentions = ()
        for i, layer_module in enumerate(self.layer): # Yue: attention transformer layers, see if they use the same attention_mask?
...
class BertForImageCaptioning(BertPreTrainedModel):
    ...
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None,
        ...
        if self.add_od_labels:
            ...
            input_ids = None
        elif self.config.got_a_generate_b:
            assert input_ids.shape[0] == batch_size
            input_ids = input_ids[:, :self.config.max_seq_a_length+1]
            od_label_ids = None
            self.od_labels_len = 0
        elif self.config.qa_as_caption:
            assert input_ids.shape[0] == batch_size
            input_ids = input_ids[:, :self.config.max_seq_a_length+1]
            od_label_ids = None
            self.od_labels_len = 0
        ...
        if not use_cbs:
            ...
            from SeeRee.modeling.utils_cbs import (ConstrainedBeamSearch,
