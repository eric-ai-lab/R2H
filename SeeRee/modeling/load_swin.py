...
from SeeRee.modeling.video_swin.swin_transformer import SwinTransformer3D
from SeeRee.modeling.video_swin.config import Config

def get_swin_model(args):
    if int(args.img_res) == 384:
        assert args.vidswin_size == "large"
        config_path = './SeeRee/modeling/video_swin/swin_%s_384_patch244_window81212_kinetics%s_22k.py'%(args.vidswin_size, args.kinetics)
        model_path = './SeeRee/modeling/video_swin_weights/swin_%s_384_patch244_window81212_kinetics%s_22k.pth'%(args.vidswin_size, args.kinetics)
    else:
        # in the case that args.img_res == '224'
        config_path = './SeeRee/modeling/video_swin/swin_%s_patch244_window877_kinetics%s_22k.py'%(args.vidswin_size, args.kinetics)
        model_path = './SeeRee/modeling/video_swin_weights/swin_%s_patch244_window877_kinetics%s_22k.pth'%(args.vidswin_size, args.kinetics)
    if args.pretrained_2d:
        config_path = './SeeRee/modeling/video_swin/swin_base_patch244_window877_kinetics400_22k.py'
        model_path = './SeeRee/modeling/video_swin_weights/swin_base_patch4_window7_224_22k.pth'
