...
def load_frames_from_files():
    ...
    # Calculate dimensions for the large image that will contain the 8x4 grid
    large_image_width = width * 4
    large_image_height = height * 8

    # Initialize an empty array for the large image
    large_image = np.zeros((large_image_height, large_image_width, 3), dtype=np.uint8)

    # Fill in the large image with the 32 smaller images
    for i in range(8):
        for j in range(4):
            x_offset = j * width
            y_offset = i * height
            large_image[y_offset:y_offset + height, x_offset:x_offset + width] = img[i * 4 + j]


    # Optionally, save the large image
...
def inference(args, model, tokenizer, tensorizer, question_txt = ''):
...
def main(args):
    ...
    inference(args, vl_transformer, tokenizer, tensorizer)
